...
static texture create_texture(int width, int height, GLenum frame_buffer,
                              GLenum texture_unit, GLenum color_attachment) {
    texture tex;
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, tex.color_attachment, GL_TEXTURE_2D,
                           tex.id, 0);
...
void create_swapper(int width, int height) {
    ...
    fbo_swapper->tex_1 = create_texture(width, height, fbo_swapper->fbo_1,
                                        GL_TEXTURE0, GL_COLOR_ATTACHMENT0);
    fbo_swapper->tex_2 = create_texture(width, height, fbo_swapper->fbo_2,
                                        GL_TEXTURE1, GL_COLOR_ATTACHMENT0);
}
